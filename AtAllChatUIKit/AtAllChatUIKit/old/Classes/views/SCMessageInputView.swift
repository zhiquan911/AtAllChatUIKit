...
@objc protocol SCMessageInputViewDelegate: AnyObject {
    ...
    *  松开手指完成录音
    */
    @objc optional func didFinishRecoingVoiceAction(voiceData: NSData, voicePath: String, voiceDuration: Float)
    ...
    - parameter sender:
    */
    @objc optional func didMediaButtonPress(inputView: SCMessageInputView)
    ...
    /**
     ...
     - parameter sender:
     */
    @objc optional func didMessageOrVoiceButtonPress(inputView: SCMessageInputView, isShowVoice: Bool)
}

public class SCMessageInputView: UIImageView, UITextViewDelegate {
    ...
    */
    ...
    func setupMessageInputViewBar() {
        ...
        if (self.allowsSendMultiMedia) {
            ...
            self.addConstraints(
                NSLayoutConstraint.constraintsWithVisualFormat(
                    "H:[multiMediaSendButton(\(self.textViewLineHeight))]-\(horizontalPadding)-|",
                    options: NSLayoutFormatOptions(),
                    metrics: nil,
        ...
        self.addConstraints(
            NSLayoutConstraint.constraintsWithVisualFormat(
                "H:|-\(testFieldMarginFront + textViewLeftMargin)-[inputTextView]-\(testFieldMarginBehind + textViewLeftMargin)-|",
                options: NSLayoutFormatOptions(),
                metrics: nil,
                views:["inputTextView": self.inputTextView]))
        ...
        if (self.allowsSendVoice) {
            ...
            self.addConstraints(
                NSLayoutConstraint.constraintsWithVisualFormat(
                    "H:|-\(testFieldMarginFront + textViewLeftMargin)-[holdDownButton]-\(testFieldMarginBehind + textViewLeftMargin)-|",
                    options: NSLayoutFormatOptions(),
                    metrics: nil,
                    views:[
    ...
    */
    func toggleMessageOrVoiceButton() {
        UIView.animateWithDuration(0.25, animations: { () -> Void in
            ...
            self.delegate?.didMessageOrVoiceButtonPress?(
                self, isShowVoice:
                self.voiceChangeButton.selected)
            self.layoutIfNeeded()
        })
    ...
    func changeKeyboardMode() {
        self.voiceChangeButton.selected = false
        self.holdDownButton.hidden = true
