...
class VoiceRecord: NSObject,AVAudioRecorderDelegate {
    ...
    var currentTimeInterval: TimeInterval = 0   //当前录音时间
    var aacPath:String { //录音存储路径
        let aacPath = NSTemporaryDirectory().appendingFormat("tmp.caf")
        return aacPath;
    }
    ...
    //音量信号幅度回调
    typealias PowerForChannel = (_ lowPassResults: Double) -> Void
    ...
    */
    private func setRecorder() {
        ...
        do {
            ...
            //初始化录音器
            self.recorder = try AVAudioRecorder(url: URL(string: self.aacPath)!, settings: [
        ...
        self.recorder?.record(forDuration: TimeInterval(self.maxRecordTime))
    ...
    */
    func startRecord(completion:() -> Void) {
        ...
        //启动定时器 定时更新录音音量
        volumeTimer = Timer.scheduledTimer(timeInterval: 0.0, target: self, selector: "levelTimer", userInfo: nil, repeats: true)
    ...
    */
    func stopRecord(completion:(_ isRecordSuccess: Bool, _ voiceData: Data?,_ voicePath: String, _ voiceDuration: Float) -> Void) {
        var isRecordSuccess: Bool
        self.stopRecord()
        let currentTime = Float(self.currentTimeInterval)
        let data: Data?   //录音文件数据
        if currentTime > minRecordTime {    //大于最小时长才录制成功
            isRecordSuccess = true
            data = try? Data(contentsOf: URL(string: self.aacPath)!)
        } else {
        ...
        //给上层提示成功或失败消息
        completion(isRecordSuccess, data, self.aacPath, currentTime)
    }
    ...
    */
    func levelTimer() {
        ...
        let averageV:Float = self.recorder!.averagePower(forChannel: 0)//获取音量的平均值
        let lowPassResults:Double = pow(Double(10), Double(0.015 * averageV))
       
        //回调音量参数给上层
        self.powerForChannel?(lowPassResults)
